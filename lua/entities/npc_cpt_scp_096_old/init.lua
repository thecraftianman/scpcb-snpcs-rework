...
ENT.ModelTable = {"models/cpthazama/scp/096_old.mdl"}
...
function ENT:SetInit()
	...
	self.IdleLoop = CreateSound(self,"cpthazama/scp/_oldscp/096idle.mp3")
	self.IdleLoop:SetSoundLevel(90) // 20
	self.TriggerLoop = CreateSound(self,"cpthazama/scp/_oldscp/096trigger.mp3")
	self.TriggerLoop:SetSoundLevel(95) // 40
	self.ChaseLoop = CreateSound(self,"cpthazama/scp/_oldscp/096chase.mp3")
	self.ChaseLoop:SetSoundLevel(110) // 6
	self.NextDoorT = 0
...
function ENT:OnThink()
	...
	if self.IsTriggered && self.CanAttack == false then
		...
		if CurTime() > self.NextTriggeredLT then
			...
			self.NextTriggeredLT = CurTime() +40
		end
...
function ENT:TriggerConfigure()
	...
	if self.IsTriggered then
		self.OverrideRunAnimation = ACT_RUN
		self.HasReset = false
...
function ENT:FindFaceLookers()
	...
	if CurTime() <= self.NextCanTriggerT then return end
	local facepos = self:GetBonePosition(14)
...
function ENT:DoAttack()
	...
	self:DoDamage(70,1000,DMG_SLASH)
