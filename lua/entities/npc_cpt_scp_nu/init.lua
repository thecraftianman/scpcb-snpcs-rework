...
ENT.ModelTable = {"models/cpthazama/scp/nu.mdl"}
ENT.StartHealth = 100
ENT.CanMutate = false
...
function ENT:Possess_Primary(possessor)
	if CurTime() > self.P_NextDirectT then
		local tb = {}
		for _,v in ipairs(ents.GetAll()) do
			if v:IsValid() && v:IsNPC() && v != self && v:GetClass() == "npc_cpt_scp_nu" && !v.IsLeader then
				table.insert(tb,v)
			end
		end
	...
	if util.IsSite19() then
		if !self.IsPossessed then
			if self.IsTakingSCP == true then
				if IsValid(self.IsTakingSCP_Box) && (util.GetSCPMapData(THESTATUE) != nil && self.IsTakingSCP_Box:GetPos():Distance(util.GetSCPMapData(THESTATUE)) <= 250) then
		...
		if CurTime() > self.NextDoorT then
			for _,v in ipairs(ents.FindInSphere(self:GetPos(),SCP_DoorOpenDistance *1.35)) do
				if v:IsValid() && v:GetClass() == "func_door" /*&& v:GetSequenceName(v:GetSequence()) == "idle"*/ then
					v:Fire("Open")
					if v.SCP_NU_FORCED == nil then v.SCP_NU_FORCED = false end
					if v.SCP_NU_FORCED == false then
						v.SCP_NU_FORCED = true
