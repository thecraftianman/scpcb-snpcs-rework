...
function ENT:Possess_OnPossessed(possessor)
	...
	possessor:ChatPrint("LMB - Command Epsilon Units")
...
function ENT:SetInit()
	...
	if table.Count(allnpcs) == 0 then
		if GetConVar("cpt_scp_ntfannouncer"):GetInt() == 1 && util.IsSCPMap() then
			if CurTime() > SCP_GlobalNTFCoolDown then
				...
				SCP_GlobalNTFCoolDown = CurTime() +SoundDuration("cpthazama/scp/ntf/Announc.mp3") +60
			end
		end
		self.IsLeader = true
		self:SetSkin(1)
	...
	if self.IsLeader && util.IsSCPMap() then
		self:Leader_Announcements()
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:Possess_Primary(possessor)
	if CurTime() > self.P_NextDirectT then
		local tb = {}
		for _,v in ipairs(ents.GetAll()) do
			if v:IsValid() && v:IsNPC() && v != self && v:GetClass() == "npc_cpt_scp_ntf" && !v.IsLeader then
				table.insert(tb,v)
			end
		end
		...
		if CurTime() > self.NextDoorT then
			for _,v in ipairs(ents.FindInSphere(self:GetPos(),SCP_DoorOpenDistance *1.35)) do
				if v:IsValid() && v:GetClass() == "func_door" /*&& v:GetSequenceName(v:GetSequence()) == "idle"*/ then
					v:Fire("Open")
					if v.SCP_NTF_Forced == nil then v.SCP_NTF_Forced = false end
					if v.SCP_NTF_Forced == false then
						v.SCP_NTF_Forced = true
			...
			if self.IsLeader == true then
				...
				if GetConVar("cpt_scp_ntfannouncer"):GetInt() == 1 then
					if didremove == true then
						...
						local glbsound
						if glbchance == 1 then
							glbsound = "surface.PlaySound('cpthazama/scp/ntf/Tesla1.mp3')"
						elseif glbchance == 2 then
							glbsound = "surface.PlaySound('cpthazama/scp/ntf/Tesla2.mp3')"
						else
							glbsound = "surface.PlaySound('cpthazama/scp/ntf/Tesla3.mp3')"
						end
						for _,v in ipairs(player.GetAll()) do
							v:SendLua(glbsound)
						end
